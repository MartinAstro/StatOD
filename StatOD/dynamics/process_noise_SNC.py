...
def process_noise(x, Q, Q_fcn, args, cse_func=cse, use_numba=True):
    ...
    if use_numba:
        Q_func = numba.njit(
            lambdify_Q, cache=False
        )  # can't cache file b/c it exists within an .egg directory
    else:
        Q_func = lambdify_Q
    ...
    Q_func(t_tmp, dt_tmp, x_tmp, Q_tmp, DCM_tmp, misc_tmp)
...
def get_Q_original(t, dt, x, Q, DCM, args):
    N = len(x)
    M = Q.shape[0]
...
def get_Gamma_SRIF(t, dt, x, Q, DCM, args):
    N = len(x)
    M = Q.shape[0]
