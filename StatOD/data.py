...
def get_measurements(filepath, t_gap=10):
    ...
    Y_station_1 = np.vstack(
        (
            ...
            measurements.get("X_obs_1_ECI", filler_obs_array).T,
        ),
    ).T
    Y_station_2 = np.vstack(
        (
            ...
            measurements.get("X_obs_2_ECI", filler_obs_array).T,
        ),
    ).T
    Y_station_3 = np.vstack(
        (
            ...
            measurements.get("X_obs_3_ECI", filler_obs_array).T,
        ),
    ).T
...
def get_measurements_pos(filepath, t_gap=10):
    ...
    # each station Y vector [(1+1+1+1+6) x len(t)]
    np.full((len(time),), np.array([np.nan]))
...
def get_scenario_1_measurements(
    filepath=os.path.dirname(os.path.abspath(StatOD.__file__))
    + "/../Data/scenario_1_observations.txt",
):
...
def get_scenario_2_measurements(file, t_gap=60):
    ...
    idx = np.zeros_like(t)
    y_station_1 = np.hstack(
        (t, idx + 0, data[:, 1 + 0].reshape((-1, 1)), data[:, 4 + 0].reshape((-1, 1))),
    )
    y_station_2 = np.hstack(
        (t, idx + 1, data[:, 1 + 1].reshape((-1, 1)), data[:, 4 + 1].reshape((-1, 1))),
    )
    y_station_3 = np.hstack(
        (t, idx + 2, data[:, 1 + 2].reshape((-1, 1)), data[:, 4 + 2].reshape((-1, 1))),
    )
...
def COEstoRV(a, e, i, RAAN, omega, nu, mu):
    p = a * (1 - e**2)
    1  # semi-latus rectum, [km]
    r = p / (1 + e * np.cos(nu))  # orbit radius, [km]

    h = np.sqrt(mu * a * (1 - e**2))  # angular momentum

    x = r * (
        np.cos(RAAN) * np.cos(omega + nu)
        - np.sin(RAAN) * np.sin(omega + nu) * np.cos(i)
    )  # x-position, [km]
    y = r * (
        np.sin(RAAN) * np.cos(omega + nu)
        + np.cos(RAAN) * np.sin(omega + nu) * np.cos(i)
    )  # y-position, [km]
    z = r * (np.sin(i) * np.sin(omega + nu))  # z-position, [km]

    xdot = x * h * e / (r * p) * np.sin(nu) - h / r * (
        np.cos(RAAN) * np.sin(omega + nu)
        + np.sin(RAAN) * np.cos(omega + nu) * np.cos(i)
    )  # x-velocity, [km/s]
    ydot = y * h * e / (r * p) * np.sin(nu) - h / r * (
        np.sin(RAAN) * np.sin(omega + nu)
        - np.cos(RAAN) * np.cos(omega + nu) * np.cos(i)
    )  # y-velocity, [km/s]
...
def get_earth_position(JD):
    ...
    Ccen = (
        (2 * e - e**3 / 4 + 5 / 96.0 * e**5) * np.sin(M)
        + (5.0 / 4 * e**2 - 11.0 / 24 * e**4) * np.sin(2 * M)
        + (13.0 / 12 * e**3 - 43.0 / 64 * e**5) * np.sin(3 * M)
        + 103.0 / 96 * e**4 * np.sin(4 * M)
        + 1097.0 / 960 * e**5 * np.sin(5 * M)
    )
