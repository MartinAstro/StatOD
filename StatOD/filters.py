...
class FilterBase(ABC):
    ...
    def gather_event_data(self, t_i, sol):
        if type(self.events) != type(None):
            ...
            if self.terminate_upon_event:
                sol.t_events[0]  # overwrite the current time step
    ...
    def run(self, t_vec, y_vec, R_vec, f_arg_vec, h_arg_vec, h_args_append=None):
        ...
        for i in range(len(t_vec)):
            ...
            self.update(t_i, y_i, R_i, f_args_i, h_args_i)
            pbar.update(i)
        pbar.close()
...
class KalmanFilter(FilterBase):
    ...
    def update(self, t_i, y_i, R_i, f_args=None, h_args=None):
        ...
        x_i, phi_i = self.propagate_forward(
            t_i,
            self.x_i_m1,
            self.phi_0,
        )  # self.phi_i_m1)
        Q_i = self.get_process_noise(t_i, x_i)
        dx_i_minus, P_i_minus = self.time_update(
            self.dx_i_m1_plus,
            self.P_i_m1_plus,
            phi_i,
            Q_i,
        ...
        if np.any(np.isnan(y_i)) or (
            ...
            dx_i_plus, P_i_plus = self.measurement_update(
                dx_i_minus,
                P_i_minus,
                K_i,
                H_i,
...
class ExtendedKalmanFilter(FilterBase):
    ...
    def update(self, t_i, y_i, R_i, f_args=None, h_args=None):
        ...
        x_hat_i_minus, phi_i = self.propagate_forward(
            t_i,
            self.x_hat_i_m1_plus,
            self.phi_0,
        )
        ...
        if (
            np.any(np.isnan(y_i))
            # or (t_i == 0.0)
            or (self.event_triggered and self.terminate_upon_event)
        ):
            x_hat_i_plus, P_i_plus = copy.deepcopy(x_hat_i_minus), copy.deepcopy(
                P_i_minus,
            )
        else:
            r_i, H_i, K_i = self.process_observations(
                x_hat_i_minus,
                P_i_minus,
                R_i,
                y_i,
            )
            x_hat_i_plus, P_i_plus = self.measurement_update(
                x_hat_i_minus,
                P_i_minus,
                K_i,
                H_i,
...
class NonLinearBatchFilter(FilterBase):
    ...
    def update(self, t_i, y_i, R_i, f_args=None, h_args=None):
        ...
        self.Lambda, self.N = self.process_observations(
            self.x_hat_i,
            self.phi_i,
            R_i,
            y_i,
    ...
    def run(self, t_vec, y_vec, R_vec, f_args_vec, h_args_vec, tol=1e-8):
        while (
            ...
            print(
                "Major Iteration: %d \t ||dx_k|| = %f"
                % (self.k, np.linalg.norm(self.dx_k)),
            )
            self.k += 1
...
class SquareRootInformationFilter(FilterBase):
    ...
    def time_update(self, dt, db_i_m1_plus, R_i_m1_plus, phi_i, Gamma_i_i_m1, mu_i_m1):
        ...
        try:
            ...
            # TSB 5.10.69 (R_k_tilde) = Pre-transform R_i_minus
            R_v_i = R_i_m1_plus @ invert(phi_i)
        ...
        # Recall that mu is the average control, not the actual control.
        ...
        R_ux_i = A_prime[
            0:q,
            q : (q + n),
        ]  # Time updated state contribution to cntr state
    ...
    def update(self, t_i, y_i, R_i, f_args=None, h_args=None, mu_i=None):
        ...
        if np.any(np.isnan(y_i)) or (
            ...
            r_tilde_i, H_tilde_i, eps_tilde_i = self.process_observations(
                x_i,
                dx_i_minus,
                P_i_minus,
                R_i,
            ...
            db_i_plus, R_i_plus, e_tilde_i_k = self.measurement_update(
                db_i_minus,
                R_i_minus,
                H_tilde_i,
                r_tilde_i,
        ...
        # measurement updated past control value (isn't used in the next t_i update (?))
        invert(R_u_i_minus) @ (db_tilde_u_i - R_ux_i @ dx_i_plus)
...
class UnscentedKalmanFilter(FilterBase):
    ...
    def propagate_forward(self, t_i, x_hat_i_m1_plus, P_i_m1_plus):
        ...
        for k in range(1, len(sigma_i_m1_plus)):
            if k < N + 1:
                sigma_i_m1_plus[k] = (
                    x_hat_i_m1_plus + self.gamma[k - 1] * sigma_vecs[:, k - 1]
                )
            else:
                sigma_i_m1_plus[k] = (
                    x_hat_i_m1_plus
                    - self.gamma[(k - 1) - N] * sigma_vecs[:, (k - 1) - (N)]
                )

        sol = solve_ivp(
            self.f_integrate,
            [self.t_i_m1, t_i],
            sigma_i_m1_plus.reshape((-1,)),
            args=(self.f, self.dfdx, self.f_args),
            atol=self.atol,
            rtol=self.rtol,
            events=event_fcn,
    ...
    def time_update(self, sigma_points, Q_i):
        ...
        for k in range(0, 2 * N + 1):
            P_i_minus += self.w_c[k] * np.outer(
                (sigma_points[k] - x_hat_i_minus),
                (sigma_points[k] - x_hat_i_minus),
            )
    ...
    def process_observations(self, x_hat_i_minus, sigma_points, y_i, R_i):
        ...
        for k in range(len(y_hat_vec)):
            P_yy_i_minus += self.w_c[k] * np.outer(
                (y_hat_vec[k] - y_hat_i_minus),
                (y_hat_vec[k] - y_hat_i_minus),
            )  # + R_i
            P_xy_i_minus += self.w_c[k] * np.outer(
                (sigma_points[k] - x_hat_i_minus),
                (y_hat_vec[k] - y_hat_i_minus),
            )
    ...
    def update(self, t_i, y_i, R_i, f_args=None, h_args=None):
        ...
        sigma_points = self.propagate_forward(
            t_i,
            self.x_hat_i_m1_plus,
            self.P_i_m1_plus,
        )
        ...
        if np.any(np.isnan(y_i)) or (
            ...
            y_hat_i_minus, K_i, P_yy_i_minus = self.process_observations(
                x_hat_i_minus,
                sigma_points,
                y_i,
                R_i,
...
class SequentialConsiderCovarianceFilter(FilterBase):
    ...
    def propagate_forward(self, t_i, x_i_m1, c_i_m1, phi_i_m1, theta_i_m1):
        ...
        M = len(c_i_m1)
        Z_i_m1 = np.hstack(
            (x_i_m1, c_i_m1, phi_i_m1.reshape((-1)), theta_i_m1.reshape((-1))),
        )
        sol = solve_ivp(
            consider_dynamics_ivp,
            [self.t_i_m1, t_i],
            Z_i_m1,
            args=(
                self.f,
    ...
    def time_update(
        ...
        # Sensitivity to measurements given consider parameters (?)
        S_xc_i_minus = phi_i @ S_xc_i_m1_plus + theta_i

        # State covariance with consider parameter covariance inflation
        P_xx_i_minus = P_i_minus + S_xc_i_minus @ P_cc_i_minus @ S_xc_i_minus.T

        # Coupling between consider parameters and state
        P_xc_i_minus = S_xc_i_minus @ P_cc_i_minus

        # Augmented State Covariance, parameters considered
        P_c_i_minus = np.block(
            [[P_xx_i_minus, P_xc_i_minus], [P_xc_i_minus.T, P_cc_i_minus]],
        )

        dx_i_minus = phi_i @ dx_i_m1_plus  # state time update

        # state time update, parameters considered
        # dx_c_i_minus = phi_i@dx_c_i_m1_plus + theta_i@dc
    ...
    def measurement_update(
        ...
        # Update covariance (parameters NOT considered)
        P_i_plus = (eye - K_i @ H_x_i) @ P_i_minus @ (
            eye - K_i @ H_x_i
        ).T + K_i @ R_i @ K_i.T
        ...
        # CKF covariance inflated by consider covariances
        P_xx_i_plus = P_i_plus + S_xc_i_plus @ P_cc_minus @ S_xc_i_plus.T
    ...
    def update(self, t_i, y_i, R_i, f_args=None, h_args=None):
        ...
        x_i, c_i, phi_i, theta_i = self.propagate_forward(
            t_i,
            self.x_i_m1,
            self.c_i_m1,
            self.phi_0,
...
class ParticleFilter(FilterBase):
    ...
    def propagate_forward(self, t_i, x_i_m1):
        ...
        sol = solve_ivp(
            dynamics_ivp_particle,
            [self.t_i_m1, t_i],
            Z_i_m1,
            args=(self.f, N, self.f_args),
            atol=self.atol,
            rtol=self.rtol,
            method="RK45",
        )
