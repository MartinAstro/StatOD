...
def get_true_trajectory(t, mu, R, cart_state, include_J3=False):
    ...
    if include_J3:
        x_original = np.hstack(
            (
                x_original,
                np.ones((len(x_original), 1)) * mu,
                np.ones((len(x_original), 1)) * J2,
                np.ones((len(x_original), 1)) * J3,
            )
        )
    else:
        x_original = np.hstack(
...
def main():
    ...
    t, Y, X_stations_ECI = get_measurements(
        "Data/Measurements/range_rangerate_w_J3_w_noise.data"
    )
    ...
    f, dfdx, dfdc = dynamics(
        x0, f_consider_J3, c_args, consider=f_consider_mask, use_numba=True
    )
    ...
    filter = SequentialConsiderCovarianceFilter(
        t0, x0, dx0, c0, dc0, P_xx_0, P_cc_0, f_dict, h_dict, logger=logger
    )
    ...
    y_labels = np.array(
        ["$x$", "$y$", "$z$", "$v_x$", "$v_y$", "$v_z$", "$\mu$", "$J_2$", "$J_3$"]
    )
    plot_state_residual_and_covariance_consider(
        logger.t_i,
    ...
    def plot_position_error(filter):
        ...
        plt.semilogy(
            np.linalg.norm(
                filter.logger.x_hat_i_plus[:, 0:3] - x_original[:, 0:3], axis=1
            ),
            label=r"$\Delta x$",
        )
        plt.semilogy(
            np.linalg.norm(
                filter.logger.x_hat_c_i_plus[:, 0:3] - x_original[:, 0:3], axis=1
            ),
            label=r"$\Delta x_c$",
        )
        ...
        plt.semilogy(
            np.linalg.norm(
                np.diagonal(filter.logger.P_i_plus[:, 0:3, 0:3], axis1=1, axis2=2),
                axis=1,
            ),
            label="$P$",
        )
        plt.semilogy(
            np.linalg.norm(
                np.diagonal(filter.logger.P_c_i_plus[:, 0:3, 0:3], axis1=1, axis2=2),
                axis=1,
            ),
            label="$P_c$",
        )
