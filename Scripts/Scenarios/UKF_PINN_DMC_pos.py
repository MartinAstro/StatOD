...
def main():
    ...
    t, Y, X_stations_ECI = get_measurements_pos(
        f"Data/Measurements/{data_file}_meas_noiseless.data",
        t_gap=30,
    )
    Y[0, 1:] = np.nan
    ...
    # Or force the process noise to update
    # def Q_forced(z0, Q0, q_fcn, Q_args, use_numba=False):
    #     Q = np.zeros((9,9))
    #     Q[0:3,0:3] = np.eye(3)*1E-6**2
    #     Q[3:6,3:6] = np.eye(3)*1E-8**2
    ...
    filter.f_integrate = (
        dynamics_ivp_no_jit  # can't pass the model into the numba JIT function
    )
    ...
    for k in range(total_batches + 1):
        ...
        X_train = (
            filter.logger.x_hat_i_plus[start_idx:end_idx, 0:3] - eros_pos[0:3]
        )  # position estimates
        Y_train = filter.logger.x_hat_i_plus[
            start_idx:end_idx,
            6:9,
        ]  # high-order accel est
    ...
    planes_exp = PlanesExperiment(
        model.gravity_model,
        model.config,
        [-model.config["planet"][0].radius * 4, model.config["planet"][0].radius * 4],
        50,
    )
