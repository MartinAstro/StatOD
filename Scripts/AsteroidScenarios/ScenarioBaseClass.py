...
class ScenarioBaseClass:
    ...
    def initializeMeasurements(self, t_vec, Y_vec, R, h_fcn, h_args_vec):
        if len(R.shape) == 1: # if diag
            ...
            raise ValueError(f"Argument R shape is not a compatible dimension: R.shape = {R.shape}")
    ...
    def initializeFilter(self, filter_class):
        ...
        filter = filter_class(self.t0, self.x0, self.dx0, self.P0, f_dict, h_dict, logger=logger)
    ...
    def run(self, train_config):
        ...
        for k in range(total_batches+1):
            ...
            end_idx = None if (k+1)*batch_size >= len(self.Y) else (k+1)*batch_size
